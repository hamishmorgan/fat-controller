...
| Code | Warning | When |
|------|---------|------|
| `W020` | Variable in both shared and service | Variable appears in shared `[variables]` and also in `[X.variables]` with a different value (can be intentional, but can also indicate an accidental shadow/override) |

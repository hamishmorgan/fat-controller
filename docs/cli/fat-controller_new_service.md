...
```text
fat-controller new service [<name>] [flags]
```
...
| `--database` | string |  |  | Database type to pre-fill (e.g. postgres, redis, mysql). |
| `--repo` | string |  |  | GitHub repo (org/name) for source. |
| `--image` | string |  |  | Docker image for source. |
...
- [fat-controller new](fat-controller_new.md) — Scaffold config entries.

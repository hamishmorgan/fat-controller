...
# fat-controller redeploy

Redeploy services from current image.

## Synopsis

```text
fat-controller redeploy [<services>] [flags]
```

## Arguments

- `<services>` — Services to redeploy (default: all).
...
| `--dry-run` | bool |  | `FAT_CONTROLLER_DRY_RUN` | Preview what would be redeployed without triggering. |

...
- **Pre-commit hooks** run gitleaks and markdown lint. Use obviously-fake values (e.g. `fakekeyfakekeyfakekey`) in test data, not anything resembling real API keys.
...
- **Settings (resources, deploy) diff** is in scope per the plan, but keep it simple — only diff fields that are explicitly set in the config file.
...
// is missing or any explicitly-specified file fails to parse.
...
// It uses a negative lookahead equivalent: match ${ not followed by {.
// Go's regexp doesn't support lookahead, so we match ${VAR} where VAR
// starts with a word character (not {).
...
func interpolateValue(val string) (string, error) {
	...
	result := localEnvPattern.ReplaceAllStringFunc(val, func(match string) string {
		...
		// Check this isn't part of a ${{...}} Railway reference.
		// Find the position in val and check if the next char after ${ is {.
		idx := strings.Index(val, match)
		if idx > 0 && idx+len(match) < len(val) {
			// This is a simple heuristic — the regex already excludes ${{
			// because VAR names start with [A-Za-z_], not {.
		}
